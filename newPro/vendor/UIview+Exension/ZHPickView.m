...
//    datePickr.locale = locale;
//    datePickr.minimumDate = [NSDate date];
    // 1.2查询所有可用的地区
    //NSLog(@"%@", [NSLocale availableLocaleIdentifiers]);
    
    // 1.3设置datePickr的地区语言, zh_Han后面是s的就为简体中文,zh_Han后面是t的就为繁体中文
    [datePickr setLocale:[[NSLocale alloc] initWithLocaleIdentifier:@"en_GB"]];
    ...
    float height = 300;
    self.frame = CGRectMake(0, SCREENSIZE.height - height - iPhoneXTabHeigth, SCREENSIZE.width, height);
    
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectZero];
    label.text = @":";
    label.textColor = [UIColor blackColor];
    [label sizeToFit];
    label.center = CGPointMake(ScreenWidth/2.0f - PXChange(10) , datePickr.centerY - 41);
    [datePickr addSubview:label];
}
