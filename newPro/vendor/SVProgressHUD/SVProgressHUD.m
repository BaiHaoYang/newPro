...
+ (void)showImage:(UIImage*)image status:(NSString*)status{
    UIImageView *svImgView = [[self sharedView] valueForKey:@"imageView"];
    CGRect imgFrame = svImgView.frame; // 设置图片的显示大小
...
+(void)showLoadingImage:(UIImage *)image status:(NSString*)status{
    UIImageView *svImgView = [[self sharedView] valueForKey:@"imageView"];
    CGRect imgFrame = svImgView.frame; // 设置图片的显示大小
    imgFrame.size = CGSizeMake(35, 35);
    svImgView.frame = imgFrame;
    NSTimeInterval displayInterval = MAXFLOAT;
    [[self sharedView] showImage:image status:status duration:displayInterval];
...
- (UIColor *)backgroundColorForStyle{
    if(self.defaultStyle == SVProgressHUDStyleLight){
        return [UIColor colorWithRed:100 green:100 blue:100 alpha:1];
        //return [UIColor whiteColor];
    } else if(self.defaultStyle == SVProgressHUDStyleDark){
        return [UIColor blackColor];
    } else{
