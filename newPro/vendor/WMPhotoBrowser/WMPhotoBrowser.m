...
- (void)viewDidLoad {
    ...
    NSString *tit = self.title?self.title:[NSString stringWithFormat:@"%ld/%ld",self.currentPhotoIndex+1,self.dataSource.count];
//    [self addCustomTitleWithTitle:tit];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if ([self.title isEqualToString:@"图片预览"]) {
        ...
        NSString *tit = [NSString stringWithFormat:@"%ld/%ld",self.currentPhotoIndex+1,self.dataSource.count];
//         [self addCustomTitleWithTitle:tit];
    }
